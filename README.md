<p align="center">
  <img src="https://github.com/rekluzlabs/rekluzlabs.github.io/blob/main/RL_transparent.png?raw=true" alt="Rekluz Labs" width="120">
</p>

<h1 align="center">Rekluz Labs Website</h1>
Source code for the [Rekluz Labs](https://rekluzlabs.github.io) website — the official landing page for Rekluz Labs, an independent Android development studio building privacy-first, ad-free apps and games.

🔗 **Live site:** [rekluzlabs.github.io](https://rekluzlabs.github.io)

---

## About

Rekluz Labs is an independent studio focused on **privacy-first, ad-free Android apps and games**, built with care and a strong attention to detail.

This repository contains the static site serving as the studio's web presence, showcasing current releases and apps in development.

---

## Technologies

- **HTML5 / CSS3**
- **JavaScript**
- **GitHub Pages** (hosting)

---

## Deployment

This site deploys automatically via GitHub Pages from the `main` branch.

Changes pushed to `main` go live at [rekluzlabs.github.io](https://rekluzlabs.github.io) within a few minutes.

---

## Contact

For inquiries, reach out at [rekluzlabs@gmail.com](mailto:rekluzlabs@gmail.com).

---

## License

**Copyright (c) 2026 Rekluz Labs. All rights reserved.**

This software and all associated files (the "Software") are the exclusive intellectual property of Rekluz Labs.

### Terms and Conditions for Use, Reproduction, and Distribution

1. **Proprietary Rights** — This Software is not open-source. All rights, title, and interest in the Software remain with Rekluz Labs.
2. **Personal Use Only** — You may download and view the source code for educational and personal purposes only.
3. **Restrictions**
   - You may not redistribute, sell, or sub-license the Software.
   - You may not create derivative works for commercial gain.
   - You may not use the branding, logo, or assets for any purpose without express written consent.

> **THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND. IN NO EVENT SHALL THE AUTHORS BE LIABLE FOR ANY CLAIM, DAMAGES, OR OTHER LIABILITY.**
